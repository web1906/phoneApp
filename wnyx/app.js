...
//(2)创建sql语句查询
var sql = "SELECT id FROM wn_yx_login WHERE uname = ? AND upwd = ?";
//(3)执行sql语句
...
server.get("/register",(req,res)=>{
  // (1)获取脚手架参数 wname wpwd phone
  var wid=req.query.wid;
  var wname=req.query.wname;
  var wpwd=req.query.wpwd;
  var phone=req.query.phone;

  // (2)创建sql语句
  var sqls=`SELECT wname FROM wn_yx_user WHERE wname='${wname}'`;
  pool.query(sqls,[wname],(err,result)=>{
    if(err)throw err;
    if(result.length>0){
      res.send({code:1,msg:"用户已存在"});
  }else{
      var sql=`INSERT INTO wn_yx_user VALUES(null,'${wname}','${wpwd}','${phone}')`;
      pool.query(sql,[wname,wpwd,phone],(err,result)=>{
        if(err)throw err;
        if(true){
    ...
    var uname=req.query.wname;
    var upwd=req.query.wpwd;
    var sqlss=`INSERT INTO wn_yx_login VALUES(null,'${uname}','${upwd}')`;
    pool.query(sqlss,[uname,upwd],(err,result)=>{
      if(err)throw err;
    })
})
//#检测   16:00
...
//6:创建sql语句
//自己写:库名;表名;列名 小写
var sql = "SELECT lid,lname,price,img_url FROM wn_yx_oder_detail LIMIT ?,?";
//7:执行sql语句
...
server.get("/shopping",(req,res)=>{
  ...
  var sql="SELECT * FROM wn_yx_shopping_detail WHERE lid=?"
...
server.get("/findcart",(req,res)=>{
  ...
  //3:创建sql查询购物车
  var sql = "SELECT lid,price,lname,count FROM wn_yx_cart WHERE uid = ?";
  //4:执行sql并且将结果返回
...
server.get("/detail",(req,res)=>{
  ...
  //3:创建sql查询购物车
  var sql = "SELECT * FROM wn_yx_oder_detail WHERE lid=?";
  //4:执行sql并且将结果返回
...
server.get("/hopcart",(req,res)=>{
  ...
  //3:创建sql查询购物车
  var sql = "SELECT * FROM wn_yx_cart";
  //4:执行sql并且将结果返回
  pool.query(sql,[uid],(err,result)=>{
    if(err)throw err;
    res.send({code:1,msg:"查询成功",data:result})
...
server.get("/del",(req,res)=>{
  ...
  if(!uid){
   ...
   return;
    ...
    var id = req.query.id;
    //2:创建sql
    var sql = "DELETE FROM wn_yx_cart WHERE id = ?";
    //3:执行
    pool.query(sql,[id],(err,result)=>{
      if(err)throw err;
